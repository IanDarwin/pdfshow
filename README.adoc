...
:releases-link: https://github.com/IanDarwin/pdfshow/releases
...
`PdfShow` has fewer features thah other tools, due to
...
For macOS and Linux, there should be an installer or two for your desktop computing platform; see
{releases-link}. Check the latest release under the _Assets_ dropdown.
...
The JARr is built with Java {current-java-ver} on most platforms.

For MS-Windows 10 or 11, there's an MSI installer in {releases-link}[the releases folder].
I'm only building for 64-bit AMD/Intel systems as that's the vast majority; others can run the jar file or build it themselves.
For those with x64 systems, you can either manually download and run the MSI installer, or simplify with winget.
Winget is MS' relatively-new package installer - they're finally catching up with every other desktop OS on the planet :-)
Anyway, if you don't have winget but do have the "Microsoft Store" app, you can get `winge`t from 
https://www.microsoft.com/p/app-installer/9nblggh4nns1#activetab=pivot:overviewtab (it's part of "Microsoft App Installer"). 
Once you have `winget` installed, just type this in a terminal window:
...
|Linux|deb|n/a|{download-link}/pdfshow_{ver}-1_amd64.deb
|Linux|rpm|n/a|{download-link}/pdfshow-{ver}-1.x86_64.rpm
|macOS|dmg|n/a|{download-link}/PDFShow-{ver}.dmg
...
The installers and the public Jar file are built with {current-java-ver}.
