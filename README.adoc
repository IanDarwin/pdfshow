...
Also, the control doesn't show the current, next, and previous slides, which is obviously lamentable
...
The JAR is built with Java {current-java-ver} on most platforms.
...
You need Java JDK, probably {current-java-ver}$$+$$, and Maven installed.
You need at least Java 14$$+$$ if you want to build any of the full platform-specific installers.
